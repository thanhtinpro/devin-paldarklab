...
USandboxExperienceDefinition* ATestExperienceActor::CreateRaidExperience()
{
    ...
    // IntrinsicTags: use pre-registered tag from SandboxExpTags
    Exp->IntrinsicTags.AddTag(SandboxExpTags::TAG_Sandbox_Mode_Raid);

    // ActionSet 1: Combat
    FSandboxActionSet CombatSet;
    CombatSet.DebugNote = TEXT("CombatLoop ActionSet");
    CombatSet.GrantedTags.AddTag(SandboxExpTags::TAG_Sandbox_Feature_Combat);
    CombatSet.GrantedTags.AddTag(SandboxExpTags::TAG_Sandbox_Feature_LagComp);
...
USandboxExperienceDefinition* ATestExperienceActor::CreateHubExperience()
{
    ...
    Exp->IntrinsicTags.AddTag(SandboxExpTags::TAG_Sandbox_Mode_Hub);

    // ActionSet: Social features, no combat
    FSandboxActionSet SocialSet;
    SocialSet.DebugNote = TEXT("Social ActionSet");
    SocialSet.GrantedTags.AddTag(SandboxExpTags::TAG_Sandbox_Feature_Trading);
    SocialSet.GrantedTags.AddTag(SandboxExpTags::TAG_Sandbox_Feature_Stable);
...
void ATestExperienceActor::Test05_ActionSets_MergeTags()
{
    ...
    USandboxExperienceManager* Mgr = CreateManager();
    Mgr->RequestExperienceLoad(TEXT("MapName?Experience=RaidSandbox"));
    FGameplayTagContainer Tags = Mgr->GetActiveExperienceTags();
    bool bHasRaid = Tags.HasTag(SandboxExpTags::TAG_Sandbox_Mode_Raid);
    bool bHasCombat = Tags.HasTag(SandboxExpTags::TAG_Sandbox_Feature_Combat);
    bool bHasLagComp = Tags.HasTag(SandboxExpTags::TAG_Sandbox_Feature_LagComp);
    bool bNoTrading = !Tags.HasTag(SandboxExpTags::TAG_Sandbox_Feature_Trading);
...
void ATestExperienceActor::Test06_SameMap_DifferentRules()
{
    ...
    bool bRaidHasCombat = RaidMgr->GetActiveExperienceTags().HasTag(
        SandboxExpTags::TAG_Sandbox_Feature_Combat);
    bool bHubHasTrading = HubMgr->GetActiveExperienceTags().HasTag(
        SandboxExpTags::TAG_Sandbox_Feature_Trading);
