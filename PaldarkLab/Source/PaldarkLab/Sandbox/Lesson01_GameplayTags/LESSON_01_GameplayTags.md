...
```mermaid
graph LR
    A[Module Load] --> B[UE_DEFINE_GAMEPLAY_TAG]
    B --> C[UGameplayTagsManager<br/>registers tag + index]
    C --> D[C++ code dùng handle<br/>SandboxTags::TAG_Sandbox_Role_Tank]
    D --> E[Compare by uint16 index<br/>O(1) thay vì O(n) string]
```
